...
//   NOTION_PAGE_ID  = 会社情報をまとめた“1ページ”のID(本文を読む/末尾に追記)
...
// ----- 会社情報：1ページ運用（本文を読む／末尾に追記） -----
const NOTION_GET_TOOL = {
  name: "get_company_info",
  description: "会社の基本情報ページの本文を読む。会社名・代表者・住所・取引銀行・口座など『会社の◯◯は？』に必ず使う。",
};
const NOTION_UPDATE_TOOL = {
  name: "update_company_info",
  description: "会社情報ページの末尾に情報を追記する(既存は消さない)。『電話番号を追加して』など会社情報の追記指示で使う。実行前に内容をユーザーへ復唱して確認すること。整理整頓は人間が行うため、AIは末尾追記のみ。",
  parameters: {
    type: "object",
    properties: { text: { type: "string", description: "追記する内容(例: 代表電話: 03-xxxx)。改行で複数行可" } },
    required: ["text"],
  },
};
async function getCompanyInfo() {
  const pageId = process.env.NOTION_PAGE_ID;
  if (!process.env.NOTION_TOKEN || !pageId) return { error: "Notionの環境変数(NOTION_PAGE_ID)が未設定です" };
  return await getPageContent({ page_id: pageId });
}
async function updateCompanyInfo({ text }) {
  const pageId = process.env.NOTION_PAGE_ID;
  if (!process.env.NOTION_TOKEN || !pageId) return { error: "Notionの環境変数(NOTION_PAGE_ID)が未設定です" };
  return await appendPageContent({ page_id: pageId, text });
}
